...
func RPCServe(electrumClient client.ElectrumClient) error {
	btcElectrumClient, ok := electrumClient.(*FiroElectrumClient)
	if !ok {
		return errors.New("electrumClient is not an instance of FiroElectrumClient")
	}
	rpc_ip := RpcDefaultIP
